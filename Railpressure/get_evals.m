...
if method == "MC" || method == "B"
    ...
    if method == "MC"
        evals = sum(csv_read(:, 3));
    elseif method == "B"
        evals = sum(csv_read(:, 4));
    end
    return
end
try
...
% if method == "AB"
%     evals = sum(samples_rep(sub2ind(size(samples_rep), (1:size(samples_rep, 1))', (csv_read(:, 4) + 1))));
% elseif method == "AMC"
%     evals = sum(samples_rep(sub2ind(size(samples_rep), (1:size(samples_rep, 1))', (csv_read(:, 3) + 1))));
