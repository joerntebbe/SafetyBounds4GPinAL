...
folders.output_ana.xtrain = importxtrain(strcat(path, '/', num2str(seeds(ii)), 'opt-all_x_train.csv'));
    ...
    plot_y1 = mean_arr(:, 1, j, k) + std_arr(:, 1, j, k);
    plot_y2 = mean_arr(:, 1, j, k) - std_arr(:, 1, j, k);
    ...
    plot(interp_arr, mean_arr(:, 1, j, k), 'k-');
